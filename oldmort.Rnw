...
The {\sc Lisa} data is yearly, with {\tt hisclass} information starting in 2002. This information, however, do not allow us to contemplate seven (or six) levels, so we are using only four classes, grouping together 1+2, 3+4, 5, and 6.


\subsection{Combining the data sets and tabulate}

<<readtab>>=
load("../CreateData/data4/vbTab.rda")
@ 
...
scap12log <- "Total hazard by HISCLASS, ages 40--59, last years."    
...
scap22log <- "Total hazard by HISCLASS, ages 60--89, last years."    
...
woutp <- plotED.glm(resw, col = scol, logScale = FALSE, ylab = "H(60|40)",
                main = "Women")
moutp <- plotED.glm(resm, col = scol, logScale = FALSE, ylab = "H(60|40)",
                main = "Men")
par(oldpar)
@
...
oldpar <- par(mfrow = c(2, 1))
wloutp <- plotED.glm(resw[-(1:6)], col = scol, logScale = FALSE, ylab = "H(60|40)",
main = "Women")
mloutp <- plotED.glm(resm[-(1:6)], col = scol, logScale = FALSE, ylab = "H(60|40)",
main = "Men")
par(oldpar)
@
...
<<lissyoung>>=
load("../CreateData/data4/liss.rda")
...
<<tablisswom1, results = "asis", cache = TRUE>>=
fitwo1 <- coxreg(Surv(enter, exit, event) ~ factor(hisclass) + civst + urban + period, 
...
<<tablissman1, results = "asis", cache = TRUE>>=
fitma1 <- coxreg(Surv(enter, exit, event) ~ factor(hisclass) + civst + urban + period, 
...
##source("R/plotED.R")
oldpar <- par(mfrow = c(2, 1))
woutp <- plotED.glm(resw, col = scol, logScale = FALSE, ylab = "H(90|60)",
                main = "Women")
moutp <- plotED.glm(resm, col = scol, logScale = FALSE, ylab = "H(90|60)",
                main = "Men")
par(oldpar)
@
...
oldpar <- par(mfrow = c(2, 1))
wloutp <- plotED.glm(resw[-(1:6)], col = scol, logScale = FALSE, ylab = "H(90|60)",
main = "Women")
mloutp <- plotED.glm(resm[-(1:6)], col = scol, logScale = FALSE, ylab = "H(90|60)",
main = "Men")
par(oldpar)
@
...
%% oldpar <- par(mfrow = c(2, 1))
%% wloutp <- plotED.ph(resw, col = scol, logScale = TRUE, ylab = "H(60|40)",
%% main = "Women")
%% mloutp <- plotED.ph(resm, col = scol, logScale = TRUE, ylab = "H(60|40)",
%% main = "Men")
%% par(oldpar)
%% @
