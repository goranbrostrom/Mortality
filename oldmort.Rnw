...
<<readtab>>=
## We want to add hisclass at 40 (hc40) and hisclass at 60 (hc60), so as a shortcut
## we start over with vb in the creation of the 'new vbTab':
##load("data/vbTab.rda") 
load("data/vb.rda")
...
vb$hisclass40 <- factor(vb$hisclass40, labels = c("1+2", "3+4", "5", "6+7"))
vb$hisclass60 <- factor(vb$hisclass60, labels = c("1+2", "3+4", "5", "6+7"))
## Now to the creation of vbTab:
vbTab <- aggregate(vb[, c("event", "exposure")],
                   vb[, c("sex", "age", "period", "cohort", "urban",
                          "civst", "hisclass40", "hisclass60")],
                   FUN = sum)
...
##
cap12 <- "Total hazard of dying before age 65 for a 40 year old person by HISCLASS and decade, women (top) and men (bottom)."
...
vb6089$hisclass <- vb6089$hisclass60
@
...
\section{Results, heart diseases \ldots}

<<causes41>>=
load("data/vb.rda")
vb <- cal.window(vb, c(1961, 2001))
x <- substr(vb$ULORSAK, 1, 2)
vb$event <- vb$event & (!is.na(x) & (x == "19"))
vb$exposure <- with(vb, exit - enter)
...
vbTab <- aggregate(vb[, c("event", "exposure")], 
                   ...
                   FUN = sum)
vbTab$hisclass <- factor(vbTab$hisclass)
vbTab$age <- factor(vbTab$age)
@ 
...
Models are fitted separately for each ten-year time period, from  1961 to
2000. We stratify on HISCLASS
and use the covariates {\tt sex}, {\tt   civst}, and {\tt urban}
...
%The distribution over time periods.
<<41getske4060>>=
cap1w <-  "Cumulative hazards for HISCLASS by time period, ages 40--64, women, causes 19."
scap1w <-  "Cumulative hazards, ages 40--64, women, causes 19."
cap1m <-  "Cumulative hazards for HISCLASS by time period, ages 40--64, men, causes 19."
scap1m <-  "Cumulative hazards, ages 40--64, men, causes 19."
##
cap12 <- "Total hazard of dying before age 60 for a 40 year old person by HISCLASS and decade, women (top) and men (bottom), causes 19."
scap12 <- "Total hazard by HISCLASS, ages 40--64, causes 19."    
cap12log <- "Total hazard of dying before age 60 for a 40 year old person by HISCLASS and decade, women (top) and men (bottom). The last decades, causes 19."
scap12log <- "Total hazard by HISCLASS, ages 40--64, last years, causes 19."    

cap2w <-  "Cumulative hazards for HISCLASS by time period, ages 65--89, women, causes 19."
scap2w <-  "Cumulative hazards, ages 65--89, women, causes 19."
cap2m <-  "Cumulative hazards for HISCLASS by time period, ages 65--89, men, causes 19."
scap2m <-  "Cumulative hazards, ages 65--89, men, causes 19."
##
cap22 <- "Total hazard of dying before age 90 for a 60 year old person by HISCLASS and decade, women (top) and men (bottom), causes 19."
scap22 <- "Total hazard by HISCLASS, ages 65--89, causes 19."    
cap22log <- "Total hazard of dying before age 90 for a 60 year old person by HISCLASS and decade, women (top) and men (bottom). The last decades, causes 19."
scap22log <- "Total hazard by HISCLASS, ages 65--89, last years, causes 19."    
...
<<41hazplotw, fig.cap = cap1w, fig.scap = scap1w, fig.height = 7>>=
source("R/plotHaz.glm.R")

plotHaz.glm(resw, main = labb, col = scol, layout = c(2, 2))
@

<<41hazplotm, fig.cap = cap1m, fig.scap = scap1m, fig.height = 7>>=
plotHaz.glm(resm, main = labb, col = scol, layout = c(2, 2))
@
...
<<41cumslogw, fig.cap = cap12log, fig.scap = scap12log, fig.height = 10>>=
oldpar <- par(mfrow = c(2, 1))
wloutp <- plotED.glm(resw, col = scol, logScale = FALSE, ylab = "H(65|40)",
main = "Women")
mloutp <- plotED.glm(resm, col = scol, logScale = FALSE, ylab = "H(65|40)",
main = "Men")
par(oldpar)
@
...
##cat("Women:\n")
drw <- drop1(fitw1, test = "Chisq")
xtable(drw, caption = "Effect of covariates for women 40-64, 1990-2005, causes 19.", label = "tab:liswom")
##round(summary(fitw1)$coef[-(1:13), ], 4)
@

The estimated effect sizes are shown in Figure \ref{fig:liswomfig}

<<41liswomfig, fig.cap = "Effect sizes, women 40-64, 1990-2005, causes 19.", fig.height = 4>>=
source("R/plotWEduc.R")
plotWEduc(fitw1)
@
...
##cat("\nMen:\n")
drm <- drop1(fitm1, test = "Chisq")
xtable(drm, caption = "Effect of covariates for men 40-64, 1990-2005, causes 19.")
##round(summary(fitm1)$coef[-(1:13), ], 4)
dcol <- c("blue", "darkgreen", "darkgreen", "red")
@

<<41lismanfig, fig.cap = "Effect sizes, men 40-64, 1990-2005, causes 19.", fig.height = 4>>=
#source("R/plotWEduc.R")
plotWEduc(fitm1)
@


<<41plotliss1, fig.cap = "Cumulative hazards, ages 40--64, 2002--2013, causes 19.", fig.height = 4>>=
#oldpar <- par(mfrow = c(1, 2))
#plot(fitw1, fn = "cum", col = dcol, main = "Women")
#plot(fitm1, fn = "cum", col = dcol, main = "Men")
#par(oldpar)
...
<<41hazplotwold, fig.cap = cap2w, fig.scap = scap2w, fig.height = 7>>=
##source("R/plotHaz.R")
plotHaz.glm(resw, main = labb, layout = c(2, 2), col = scol)
@

<<41hazplotmold, fig.cap = cap2w, fig.scap = scap2m, fig.height = 7>>=
plotHaz.glm(resm, main = labb, layout = c(2, 2), col = scol)
@
...
<<41cumslogwm, fig.cap = cap22log, fig.scap = scap22log, fig.height = 10>>=
oldpar <- par(mfrow = c(2, 1))
wloutp <- plotED.glm(resw, col = scol, logScale = FALSE, ylab = "H(90|65)",
main = "Women")
mloutp <- plotED.glm(resm, col = scol, logScale = FALSE, ylab = "H(90|65)",
main = "Men")
par(oldpar)
@
...
##cat("Women:\n")
drw2 <- drop1(fitw2, test = "Chisq")
xtable(drw2, caption = "Effect of covariates for women 65-89, 1990-2005, causes 19.", label = "tab:liswom")
##round(summary(fitw1)$coef[-(1:13), ], 4)
@

The estimated effect sizes are shown in Figure \ref{fig:liswomfig}

<<41liswomfig2, fig.cap = "Effect sizes, women 65-89, 1990-2005, causes 19.", fig.height = 4>>=
#source("R/plotWEduc.R")
plotWEduc(fitw2)
@
...
##cat("\nMen:\n")
drm2 <- drop1(fitm2, test = "Chisq")
xtable(drm2, caption = "Effect of covariates for men 65-89, 1990-2005, causes 19.")
##round(summary(fitm1)$coef[-(1:13), ], 4)
dcol <- c("blue", "darkgreen", "darkgreen", "red")
@

<<41lismanfig2, fig.cap = "Effect sizes, men 65-89, 1990-2005, causes 19.", fig.height = 4>>=
#source("R/plotWEduc.R")
plotWEduc(fitm2)
@
