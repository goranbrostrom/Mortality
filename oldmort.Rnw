...
\author{Göran Broström\footnote{email: \texttt{goran.brostrom@umu.se}} and
    Sören Edvinsson\footnote{email: \texttt{soren.edvinsson@umu.se}} \\
...
<<setup, include = FALSE>>=
knitr::opts_chunk$set(cache=TRUE,echo=FALSE,message=FALSE,warning=TRUE,comment=NA)##$
@
...
<<readtab>>=
load("data/vbTab.rda") 
hisclevels <-  c("elite", "middle", "worker") # NOTE!!
vbTab$period <- factor(vbTab$period)
levels(vbTab$hisclass40) <- hisclevels
levels(vbTab$hisclass60) <- hisclevels
levels(vbTab$period)[8] <- "1991-2001"
load("data/vb.rda")
vb$period <- factor(vb$period)
levels(vb$hisclass) <- hisclevels
levels(vb$hisclass40) <- hisclevels
levels(vb$hisclass60) <- hisclevels
levels(vb$period)[8] <- "1991-2001"
@ 

\section{Introduction}

For a long time, it was generally assumed that socio-economic health inequalities were high in historical societies, probably higher than in modern societies. This can be considered as a reasonable assumption since these societies were in most cases characterised by very large differences between groups and classes. Knowing that access to resources provides advantages in all ways, the health advantage of higher classes ought to be obvious. When in recent years new studies have appeared, investigating social inequality in health and mortality with micro-data, the assumed pattern has been questioned. Results from different studies have diverged, some reporting substantial differences but many have found surprisingly small socio-economic differences during the first stages of the transition \citep{bevp11, edli11,edbr12}. The appearance of the consistent pattern of inequality in survival that characterises present-day societies must have developed somewhere on the road from a high-mortality to a low-mortality regime. The complete process is however largely unknown. 

A long-term perspective on the development of social
inequalities in mortality in the adult and elderly population is taken in the present paper. 
The area under investigation consists of the Skellefteå and Umeå regions in
...
population during the mortality transition. The
analyses will be performed on data from the Skellefteå and Umeå regions
1851--2013. The main questions are:
...
\item Are there large differences in survival depending on level of education and income categories?
\end{enumerate}
...
\begin{figure}
\includegraphics[height=19cm]{myfigs/skum17}
...
\item {\tt lowMan}, HISCLASS 3 and 8. Lower managers and farmers.
\item {\tt lowWhiteC}, HISCLASS 4, 5 and 6. Lower white collar.
...
<<hiscskum, fig.cap = "Exposure by hisclass (from the top: elite, farmers and lower business, lower white collar, workers) and time period", fig.height = 4.5>>=
xx <- with(vbTab, tapply(exposure, list(period = period, hisclass = hisclass40), sum))
yy <- round(prop.table(xx, 1) * 100, 1)
who <- length(levels(vbTab$hisclass40))
barplot(t(yy)[who:1, ], col = c("red", "blue", "black"), las = 2, ylab = "Per cent")
@
...
library(xtable)
load("data/vb0.rda")
vb0$hisclass <- factor(vb0$hisclass, labels = hisclevels)
...
xx <- with(vbTab, tapply(exposure, list(period = period, civst = civst), sum))
yy <- round(prop.table(xx, 1) * 100, 1)
...
woutp <- plotED.glm(resw, col = scol, logScale = TRUE, ylab = "log(1-S(65|40))",
                main = "Women 40-64")##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = TRUE, ylab = "log(1-S(65|40))",
...
%The distribution over time periods.
<<getske6089>>=
vb6589 <- vbTab[as.numeric(as.character(vbTab$age)) >= 64.5, ]
