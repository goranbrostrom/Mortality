...
Models are fitted separately for each ten-year time period, from  1981 to
...
##
cap12 <- "Total hazard of dying before age 60 for a 40 year old person by HISCLASS and decade, women (top) and men (bottom), cardiovascular death."
...
##
cap22 <- "Total hazard of dying before age 90 for a 60 year old person by HISCLASS and decade, women (top) and men (bottom), cardiovascular death."
scap22 <- "Total hazard by HISCLASS, ages 65--89, cardiovascular death."    
cap22log <- "Total hazard of dying before age 90 for a 60 year old person by HISCLASS and decade, women (top) and men (bottom). The last decades, cardiovascular death."
...
##cat("Women:\n")
drw <- drop1(fitw1, test = "Chisq")
xtable(drw, caption = "Effect of covariates for women 40-64, 1990-2005, cardiovascular death.", label = "tab:liswom")
##round(summary(fitw1)$coef[-(1:13), ], 4)
@
...
<<41getske6089>>=
vb6589 <- vbTab[as.numeric(as.character(vbTab$age)) >= 64.5, ]
vb6589$age <- factor(vb6589$age) # NOTE:: Now change to '6589'!
@
...
##cat("Women:\n")
drw2 <- drop1(fitw2, test = "Chisq")
xtable(drw2, caption = "Effect of covariates for women 65-89, 1990-2005, cardiovascular death.", label = "tab:liswom")
##round(summary(fitw1)$coef[-(1:13), ], 4)
@
