...
We use the *expected remaining years of life at 65 up to age 100*  as the summary measure, since the relative risk is a not so good measure when risks are high. First, looking at Table \@ref(tab:cumswtabold), it is obvious that the statistical significance regarding differences between groups over time is still strong.
...
The absolute years remaining are shown in Figure \@ref(fig:cumsw2abs).

```{r cumsw2abs, fig.cap = "Remaining years of life before 100 for a 65-year-old, by social class and period.", fig.scap = "Differences in expected years of life at 65", fig.height = 8, out.height = "14cm"}
##source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
mm <- 0 ##resw[3, ]
...
The excess years of remaining life compared to worker is shown in Figure \@ref(fig:cumsw2).
...
The general pattern and development of mortality do not
differ much from the younger age group, but on a much
...
Concerning the differences in the social pattern (Figure \@ref(fig:cumsw2)), it is less clear compared to what
we find for the younger age group in the earlier time periods. For women the results are mixed. We do not find the
...
20th century. It is only during the four last periods (after the year 1975) that it is
