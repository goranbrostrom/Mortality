...
date: 'Centre for Demographic and Ageing Research \newline Umeå University, SE-90187 Umeå, Sweden `r Sys.time()`'
output:
...
The adjustment carried out by inverse probability weighting [@hoth52;@hero06] in the following way: First, for each social class, 
...
-->
Differences in mortality in the age group 40--64 are getting very small as we move forward in time: Almost all forty-year-olds today survive beyond 65, regardless of social class. In Sweden of the year 2014, the probability of surviving age 65 for a forty-year-old man is 91 per cent  (a period measure). The corresponding figure for a forty-year-old woman is 94 per cent. A more relevant measure of comparison is the probability (risk) of dying before age 65 for a forty-year-old, remembering that the absolute risks are very small in the later time periods. 
...
absolute differences between social classes in the later time periods. We must, however, distinguish between differences in absolute risks compared to relative risks [@hoetal16]. The compression that is visible for later decades is partly explained by the decreasing general mortality and does not necessarily represent a homogenisation of relative risks.

By standardizing the levels in each time period for each class, we illustrate the relative risks in Figure \@ref(fig:probsw).
...
We use the *expected remaining years of life at 65 up to age 100* (reported as medan age at death) as the summary measure, since the relative risk is a not so good measure when risks are high. First, looking at Table \@ref(tab:cumswtabold), it is obvious that the statistical significance regarding differences between groups over time is still strong.
...
```{r cumsw2abs, fig.cap = "Median age at death for a 65-year-old, by social class and period.", fig.scap = "Differences in expected years of life at 65", fig.height = 8, out.height = "14cm"}
##source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
mm <- 0 ##resw[3, ]
...
The general pattern and development of mortality resembles that from the younger age group (notice that the comparisons are made with different methods in the two age groups, and the figures are therefore only comparable for the identification of patterns in the development), according to the median age at death for a 65-year-old shown in Figure \@ref(fig:cumsw2abs). Median death age among women, as expected, was higher than among men. We find a clear increase in life expectancy, but
