...
The social classes are compared by the \emph{expected years of life
before 65(100)} for a person aged 40(65). This measure is calculated
as the area under the survival curve for each class.
...
A more relevant measure of comparison is perhaps the probability to die before age 65 for a forty-year-old, see Figure \@ref(fig:probsw).

```{r probsw, fig.cap = "Relative probability of dying before the age of 65, given survival to age 40, by period and hisclass. Reference is worker.", fig.scap = "Relative probability of dying before 65.", fig.height = 8, out.height = "14cm"}
source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
refclw <- pdw[3, ]
plot(midper, pdw[1, ] / refclw, col = scol[1], type = "b", ylim = c(1/3, 3), 
     ...
     ylab = "Relative probability", axes = FALSE, main = "Women, 40-64", lty = 1, pch = 1)
...
plot(midper, pdm[1, ] / refclm, col = scol[1], type = "b", ylim = c(1/3, 3), xlab = "Period", 
     ylab = "Relative probability", axes = FALSE, main = "Men, 40-64", lty = 1, pch = 1,
...
Figure \@ref(fig:cumsw) shows the differences in expected years of survival in the age interval
...
absolute differences between social classes. We must, however, distinguish between differences in absolute risks and relative risks. The compression that is
visible for later decades is partly explained by the decreasing general
mortality and does not imply decreasing relative risks, see Figure \@ref(fig:probsw). 
