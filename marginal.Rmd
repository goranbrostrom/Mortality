...
author: X \footnote{Corresponding author; Phone number +0000000000, email X@xcom.com}
  and Y
date: '`r Sys.Date()`'
output:
...
library(knitr)
opts_chunk$set(engine='R',fig.path='figs/art3d-',tidy=FALSE,fig.height=8.5,fig.width=8,strip.white=TRUE,cache=TRUE,echo=FALSE,message=FALSE,comment=NA,show.signif.stars = FALSE,warning = FALSE)##$
library(knitr)
library(eha)
library(xtable)
library(skum)
...
source("R/makeTab.R")
vb1Tab <- makeTab(vb1)
source("~/R/Testing/pch.R")
Tfit <- pch(event ~ offset(log(exposure)) + age + hisclass * period * sex + civst + urban, seq(40, 65, by = 5), vb1Tab)
Tdr <- drop1(Tfit, test = "Chisq")
knitr::kable(Tdr, caption = "ANOVA, ages 40-65.")
