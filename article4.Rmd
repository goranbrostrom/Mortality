...
vb0Tab <- aggregate(vb0Tab[, c("event", "exposure")],
                    ...
                    FUN = sum)
vb0Tab$hisclass <- relevel(vb0Tab$hisclass, ref = "middle")
```
...
```{r mapp,fig.cap = "The Skelleftea and Umea regions in Sweden.", out.width="3in", fig.height = 16.1, fig.align="center"}
knitr::include_graphics("myfigs/sweden.pdf")
```
...
```{r hiscskumm, fig.cap = "Exposure by social class (from the top: elite, middle, workers, missing) and time period, men.", fig.scap="Exposure by social class and time period, men.", out.height = '15cm'}
...
```{r hiscskumw, fig.cap = "Exposure by social class (from the top: elite, middle, workers, missing) and time period, women.", out.height = '15cm', fig.scap="Exposure by social class and time period, women."}
...
```{r civstskum, fig.cap = "Exposure by marital status (from the top: never married, married and previously married) and time period", fig.scap='Exosure by marital status and time period.',out.height = '24.5cm'}
...
scol <- c("blue", "darkgreen", "black", "red")
...
scap0 <- "Probability of dying before age 65 for a 40-year old by social class."
cap1 <- "Probability of dying before age 90 for a 65-year-old by social class and period, women (top) and men (bottom)."
scap1 <- "Probability of dying before age 90 for a 65-year old by social class."
cap12 <- "Relative probability of dying before age 65 for a 40-year-old by social class and period, women (top) and men (bottom)."
scap12 <- "Relative probability of dying before age 65 for a 40-year-old by social class."
...
scap22 <- "Relative probability of dying before age 90 for a 65-year-old by social class."
...
##nper <- 4
nper <- length(labb)
reswh <- matrix(0, nrow = 4, ncol = nper)
resmh <- matrix(0, nrow = 4, ncol = nper)
for (i in 1:nper){
  reswh[, i] <- preg("+ civst + urban", data = vb4064[vb4064$sex == "female" &
                                               vb4064$period == labb[i], ])
    resmh[, i] <- preg("+ civst + urban", data = vb4064[vb4064$sex == "male" &
                                               vb4064$period == labb[i], ], debug = FALSE)
}
...
xx <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1977.5, 1982.5, 1987.5, 1992.5, 1998.5, 2004.5, 2010.5)
plot(xx, reswh[1, ], col = scol[1], type = "b", pch = 1, lty = 3, ylim = c(0.0, 4), main = "Women 40-64")
for(i in 2:4){
   lines(xx, reswh[i, ], col = scol[i], type = "b", lty = (i == 2) + (i != 2) * 3, pch = i)
}
plot(xx, resmh[1, ], col = scol[1], type = "b", pch = 1, lty = 3, ylim = c(0.0, 4), main = "Men 40-64")
for(i in 2:4){
   lines(xx, resmh[i, ], col = scol[i], type = "b", lty = (i == 2) + (i != 2) * 3, pch = i)
}
par(oldpar)
```
...
##source("R/plotED.R")
source("R/getPHglm.R")
resw <- getPHglm(vb6589[vb6589$sex == "female", ],
            nper, labb)
resm <- getPHglm(vb6589[vb6589$sex == "male", ],
            nper, labb)

oldpar <- par(mfrow = c(2, 1))
woutp <- plotED.glm(resw, col = scol, logScale = FALSE, relative = FALSE,
                    ylab = "P(dead at 90 | alive at 65)", ylim = c(0.4, 1.0), main = "Women 65-89")##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = FALSE, relative = FALSE,
                    ylab = "P(dead at 90 | alive at 65)", ylim = c(0.4, 1.0), main = "Men 65-89")##, exclude = 5:6)
par(oldpar)
```
