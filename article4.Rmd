...
##nper <- 4
nper <- length(labb)
resw <- matrix(0, nrow = 4, ncol = nper)
resm <- matrix(0, nrow = 4, ncol = nper)
for (i in 1:nper){
  resw[, i] <- preg("+ civst", data = vb4064[vb4064$sex == "female" &
                                               vb4064$period == labb[i], ])
    resm[, i] <- preg("+ civst", data = vb4064[vb4064$sex == "male" &
                                               vb4064$period == labb[i], ])
}
##resw <- getPHglm(vb4064[vb4064$sex == "female", ],
##            nper, labb)
##resm <- getPHglm(vb4064[vb4064$sex == "male", ],
##            nper, labb)
```
...
woutp <- plotED.glm(resw, col = scol, logScale = FALSE, relative = TRUE,
                    ylab = "P(dead at 65 | alive at 40)", ylim = c(0.0, 0.6), main = "Women 40-64")##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = FALSE, relative = TRUE,
                    ylab = "P(dead at 65 | alive at 40)", ylim = c(0.0, 0.6), main = "Men 40-64")##, exclude = 5:6)
par(oldpar)
```
