...
\item Development over \emp{time}.
  \begin{itemize}
\item Explicit or implicit assumption of an \epm{eternal} social gradient in history in many theories (Omran, Link and Phelan, Marmot, McKeown), some implying even larger in history.
