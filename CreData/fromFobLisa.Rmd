...
save(fobdata, file = "fobdata2.rda")
```
...
```{r syskonfob}
    ...
    ## Instead
    ##fobb <- group_by(fobb, LINNEID) %>% fill_("hisclass") # Later!!summary(lisa)
    fobb$id <- fobb$LINNEID
    ##fobb <- addPer(fobb, cuts = c(1961, 1976, 1986, 1996, 2002)) # Wait!
    load("../../Data/sex.rda")
...
##save(lisadata, file = "lisadata2.rda")
```
...
```{r lisadata}
    ...
    ## Push forward:
    source("R/pushHis.R")
    ## liss <- pushHis(lisadata) # Later!
    liss <- lisadata
    ##liss$hisclass <- liss$nyhisclass
    ##liss$nyhisclass <- NULL
    ## Now:
    ##liss <- group_by(lisadata, LINNEID) %>% fill_("hisclass") # Later!!
    liss$id <- liss$LINNEID
    ##lisaold$id <- lisaold$LINNEID

    ## lisaold <- addPer(lisaold, name = "period", cuts = seq(1986, 2014, by = 2)) ## Later! 
    ## liss <- addPer(liss, name = "period", cuts = seq(2002, 2014, by = 6)) # Later!!
...
foli$hisclass <- factor(foli$hisclass, labels = c("elite", "middle", "worker", "farmer", "none"))
foli$hisclass40 <- fixCovar2(foli$id, foli$hisclass, foli$enter, foli$exit, age = 40, lookBack = 20)
foli$hisclass65 <- fixCovar2(foli$id, foli$hisclass, foli$enter, foli$exit, age = 65, lookBack = 45)
save(foli, file = "foli.rda")
```
