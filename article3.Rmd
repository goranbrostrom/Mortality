...
sources. In Poplink, occupations are coded according to the original DDB coding system that has then been adapted to Hisclass. For the Linnaeus data, HISCO is recoded into Hisclass. The censuses include occupational codes based on the system used in Sweden at the time, NYK (Nordisk
Yrkesklassificering). We consider the observed social class in a census to be valid for the full period until the next census, corresponding to our definition of presence periods. For the period 1991--2000 the social class in the 1990 census is considered to be maintained until 2001. 


The use of different data sets forces us to apply different approaches
to defining the social position. Poplink usually only provides the occupation of the head of household, thus underestimating female labour force participation as well as that of
adult children residing with their parents [@vik10]. For the Poplink period, we have chosen to categorize wives according to the position of the head of household, usually
...
complication, however, is that it is  difficult to define households in the
same way as in Poplink. Extramarital cohabitation became common and female
...
different periods are not completely comparable. Noti, however, that there was strong social homogamy within married couples at this time, indicating that the results would not differ in any decisive way with the use of occupations on individual level instead of household level for defining social class based on the Linneaus data [@bjork92]. 
...
economic and other resources, status, social power, etc. Longitudinal analyses of social classes for such a long period covered in this study present several problems, related to changes in occupational structure and the meaning and content of occupations and social positions. We have chosen to work with a broad definition of classes, merging the original 12 classes in Hisclass into the following three and an additional fourth category of unknown class:
...
not apparent in Figure \@ref(fig:hiscskumm) is the transfer within the middle
...
file. The statuses used are unmarried, married and dissolved marriage
...
estimation of \@ref(eq:prop) can be performed in two ways; based on the
...
##
cap0 <- "Probability of dying before age 65 for a 40-year-old by social class and period, women (top) and men (bottom)."
scap0 <- "Probability by social class, ages 40--64."
cap12 <- "Probability of dying before age 65 for a 40-year-old standardized by social class and period, women (top) and men (bottom)."
scap12 <- "Probability standardized by social class, ages 40--64."
...
##
cap22 <- "Probability of dying before age 90 for a 65-year-old by social class and period, women (top) and men (bottom). Standardised."
...
Looking at Table \@ref(tab:cumswtab) it is obvious that the statistical significance is strong.
...
duration 25 (age 90), starting from age 65 as a summary measure. Looking at Table \@ref(tab:cumswtabold) it is obvious that the statistical significance is still strong.
...
time. In most periods the social differences were small, particularly between the workers and the middle group, and the elite even had somewhat higher mortality during the early 20th century. The social differences continued to be small until the last decades of the
