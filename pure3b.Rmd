---
title: "Is high social class always beneficial for survival? Northern Sweden 1801--2013 (cohort version)"
...
##
su <- rbind(skel[, c("id", "birthdate", "sex", "period", "civst", "hisclass", "enter", "exit", "event")], 
            skume[, c("id", "birthdate", "sex", "period", "civst", "hisclass", "enter", "exit", "event")])
su$id <- paste("s", su$id, sep = "_")
su <- rc(su)
Exit <- tapply(su$exit, su$id, max)
Event <- tapply(su$event, su$id, max)
idx <- tapply(su$id, su$id)
su$Exit <- Exit[idx]
su$Event <- Event[idx]
su <- su[su$lopnr == 1, ]
su$exit <- su$Exit
...
foli <- foli[, c("id", "birthdate", "sex", "period", "civst", "hisclass", "enter", "exit", "event")]
foli <- rc(foli)
foli$id <- paste("f", foli$id, sep = "_")
Exit <- tapply(foli$exit, foli$id, max)
Event <- tapply(foli$event, foli$id, max)
idx <- tapply(foli$id, foli$id)
foli$Exit <- Exit[idx]
foli$Event <- Event[idx]
foli <- foli[foli$lopnr == 1, ]
foli$exit <- foli$Exit
foli$event <- foli$Event

vb0 <- rbind(su, foli)
rownames(vb0) <- 1:NROW(vb0)

vb0 <- vb0[vb0$enter <= 40, ]  ## NOTE!!
...
```{r hiscskumm, fig.cap = "Exposure by social class (from the top: elite, middle, workers, farmers, missing) and time period, men.", fig.scap="Exposure by social class and time period, men.", out.height = '50%', fig.height=4}
...
```{r ipw}
head(vb0)
da <- vb0[vb0$sex == "male" & vb0$period == "1876-1900", c("id", "civst", "hisclass", "enter", "exit", "event")]
gg <- as.numeric(da$hisclass)
lfit1 <- glm(gg == 1 ~ civst, data = da, family = "binomial")
p1 <- predict(lfit1, type = "response")
lfit2 <- glm(gg == 2 ~ civst, data = da, family = "binomial")
p2 <- predict(lfit2, type = "response")
lfit3 <- glm(gg == 3 ~ civst, data = da, family = "binomial")
p3 <- predict(lfit3, type = "response")
lfit4 <- glm(gg == 4 ~ civst, data = da, family = "binomial")
p4 <- predict(lfit4, type = "response")
lfit5 <- glm(gg == 5 ~ civst, data = da, family = "binomial")
p5 <- predict(lfit5, type = "response")
tmp <- p1
tmp[gg==2] <- p2[gg==2]
tmp[gg==3] <- p3[gg==3]
tmp[gg==4] <- p4[gg==4]
...
raw <- rbind(rawskel, rawskume, rawfoli)
