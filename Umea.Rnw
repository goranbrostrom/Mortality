...
\begin{columns}
\begin{column}{0.6\textwidth}
<<picklexus3,fig.width=6,fig.height = 5.9>>=

##load("data/vb.rda")
tot.m <- length(unique(vb$id[vb$dataset != "poplink" & vb$sex == "male"]))
tot.f <- length(unique(vb$id[vb$dataset != "poplink" & vb$sex == "female"]))

vbd <- vb[vb$event == 1 & vb$dataset != "poplink", c("birthdate", "exit", "sex")]
d.m <- sum(vbd$sex == "male")
d.f <- sum(vbd$sex == "female")
...
<<anovatable,results='asis',echo=FALSE>>=
xtable(dr)
@
...
{\scriptsize
<<extab,echo=FALSE,results='asis'>>=
xtable(summary(fitp)$coefficients[1:10, 1:2])
@
}
