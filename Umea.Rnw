...
\title{Long--term perspectives of social inequality in mortality among adults and elderly in
  northern  Sweden 1851--2013\footnote{Session "Health and Mortality in Arctic Populations" at The Ninth International Congress on Arctic Social Sciences (ICASS IX) June 11, 2017, Umeå}
}

\date{June 11, 2017}

\author{Sören Edvinsson and Göran Broström \\
Centre for Ageing and Demographic Research, Umeå University}
...
<<setup, include = FALSE>>=
library(knitr)
opts_chunk$set(engine='R',dev='pdf',fig.path='figs/ume-',tidy=FALSE,fig.height=5.5,fig.width=8,strip.white=TRUE,cache=TRUE,echo=FALSE,message=FALSE,comment=NA,show.signif.stars = FALSE)##$
@
...
Remote area in northern Sweden, dominated by agriculture until well into the 20th century. 19th century communications were rudimentary.


Skellefteå:
\begin{itemize}

\item Small town, industrialization of the region during the 20th century.
...
\item[Education] 1990--2005, three categories.
...
\begin{frame}[fragile]{Women and men, age 40--64}
...
\begin{frame}[fragile]{Relative differences, ages 40--64 and 65--89}

  %\begin{center}
\includegraphics[height=7.5cm,width=5.4cm]{figs/art2-cumswstand-1}\includegraphics[height=7.5cm,width=5.4cm]{figs/slide-cumswold-1}
   %\end{center}
\end{frame}
...
\begin{frame}[fragile]{Income 1986--2005}
...
\item In the long run, no clear connection between income inequality and social differences in mortality.
\end{itemize}
