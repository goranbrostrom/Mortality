...
vb0Tab <- aggregate(vb0Tab[, c("event", "exposure")],
                    ...
                    FUN = sum)
vb0Tab <- vb0Tab[!(vb0Tab$age %in% c(30, 35)), ]
```
...
## Median remaining life at age 40
...
##
cap0m <- "Median age at death for a 40-year-old by social class and period, women (top) and men (bottom)."
scap0m <- "Median age at death for a 40-year old by social class."
cap0md <- "Differences in median age at death for a 40-year-old by social class and period (ref: middle class), women (top) and men (bottom)."
scap0md <- "Differences in median age at death for a 40-year old by social class."
...
##
vb40 <- vb0Tab
vb40$age <- factor(vb40$age)
##vb40$hisclass <- vb40$hisclass40
```

```{r hazcalw}
source("R/pgetPHglm2.R")
source("R/getPHglm.R")
##nper <- 4
nper <- length(labb)
resw <- pgetPHglm2(vb40[vb40$sex == "female", ],
            nper, labb)$medi
resm <- pgetPHglm2(vb40[vb40$sex == "male", ],
            nper, labb)$medi
```


```{r meli, fig.cap = cap0m, fig.height = 8, out.height = "14cm"}
oldpar <- par(mfrow = c(2, 1))
plot(1:13, resw[1, ], type = "b", col = scol[1], axes = FALSE, ylim = c(60, 95), main = "Women", ylab = "Age", xlab = "", xlim = c(1, 15.5))
axis(1, at = 1:13, labels = labb, las = 2)
axis(2, at = c(60, 70, 80, 90), las = 1)
abline(h = c(60), lty = 3)
abline(h = c(70, 80, 90), lty = 3)
box()
lines(1:13, resw[2, ], type = "b", lty = 2, pch = 2, col = scol[2])
lines(1:13, resw[3, ], type = "b", lty = 3, pch = 3, col = scol[3])
lines(1:13, resw[4, ], type = "b", lty = 4, pch = 4, col = scol[4])
abline(h = 60)
legend("bottomright", legend = c("elite", "middle", "worker", "NA"), lty = 1:4, pch = 1:4, col = scol)
## Men:
plot(1:13, resm[1, ], type = "b", col = scol[1], axes = FALSE, ylim = c(60, 95), main = "Men", ylab = "Age", xlab = "", xlim = c(1, 15.5))
axis(1, at = 1:13, labels = labb, las = 2)
axis(2, at = c(60, 70, 80, 90), las = 1)
box()
lines(1:13, resm[2, ], type = "b", lty = 2, pch = 2, col = scol[2])
lines(1:13, resm[3, ], type = "b", lty = 3, pch = 3, col = scol[3])
lines(1:13, resm[4, ], type = "b", lty = 4, pch = 4, col = scol[4])
abline(h = 60)
abline(h = c(70, 80, 90), lty = 3)
legend("bottomright", legend = c("elite", "middle", "worker", "NA"), lty = 1:4, pch = 1:4, col = scol)
par(oldpar)
source("R/plotED.glm.R") # From below (not eval)
```

In Figure \@ref(fig:melidiff) the difference in median life (Reference: middle class) is shown.

```{r melidiff, fig.cap = cap0md, fig.height = 8, out.height = "14cm"}
oldpar <- par(mfrow = c(2, 1))
for (m in c(1, 3, 4)) resw[m, ] <- resw[m, ] - resw[2, ]
resw[2, ] <- 0
for (m in c(1, 3, 4)) resm[m, ] <- resm[m, ] - resm[2, ]
resm[2, ] <- 0
plot(1:13, resw[1, ], type = "b", col = scol[1], axes = FALSE, main = "Women", ylab = "Years", xlab = "", xlim = c(1, 15.5), ylim = c(-8, 8))
axis(1, at = 1:13, labels = labb, las = 2)
...
legend("bottomright", legend = c("elite", "middle", "worker", "NA"), lty = 1:4, pch = 1:4, col = scol)
## Men:
plot(1:13, resm[1, ], type = "b", col = scol[1], axes = FALSE, main = "Men", ylab = "Years", xlab = "", xlim = c(1, 15.5), ylim = c(-8, 8))
axis(1, at = 1:13, labels = labb, las = 2)
axis(2, at = c(-5, 0, 5), las = 1)
box()
lines(1:13, resm[2, ], type = "b", lty = 2, pch = 2, col = scol[2])
...
source("R/plotED.glm.R") # From below (not eval)
```
```{r cumsw, fig.cap = cap0, fig.scap = scap0, fig.height = 8, out.height = "14cm",eval = FALSE}
...
vb40$social_class <- vb40$hisclass
vb40$marital_status <- vb40$civst
vb40$urban_or_rural <- vb40$urban
vb40$age_group <- vb40$age
fitt <- glm(event ~ offset(log(exposure)) + urban_or_rural + marital_status + age_group + sex * social_class * period,
            data = vb40, family = poisson)
...
```{r cumswstand, fig.cap = cap12, fig.scap = scap12, fig.height = 8, out.height = "14cm",eval = FALSE}
oldpar <- par(mfrow = c(2, 1))
