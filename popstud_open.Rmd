---
title: Is high social class always beneficial for survival? A study of northern Sweden 1801--2013 \footnote{Short; "Is high social class beneficial for survival?"} \footnote{Funding:This work was founded by the Swedish Research Council under grants No 349-2008-6592 No 421-2012-892}
...
- \subsectionfont{\normalfont\itshape}
bibliography: mybib
abstract: "Focusing on two regions in northern Sweden 1801--2013, we challenge common
  ...
  social class and health is more complex than is assumed in many of the dominant
    theories in demography and epidemiology.\\par\n\\textbf{Keywords:} Class reversal
  in mortality; Cox regression; Gender differences; Inverse probability weighting;
  Mortality transition; Northern Sweden; Social inequality; 19th to 21 century."
...
The data for the present study come from two large population databases at the Demographic Data Base (DDB),Umeå University (http://www.cedar.umu.se), which provide us with micro-data for the Skellefteå and Umeå regions in northern Sweden. The period 1801--1950 is covered by the database Poplink [@wee16]. Poplink is based on linked parish records, allowing us to reconstruct life biographies on people as long as they remained in the region. The records are linked within, but not
between, the regions.
...
respectively (see also Appendix). The farmer category was dominant until the middle of the 20th centuries but have become marginal in the latest periods, particularly among women. The category of workers is fairly stable over time. The highest class has increased substantially, albeit from a very low level
...
The proportion with missing values among women is a larger problem, however. Corresponding to what we found for men, the
...
The social classes are compared by the *expected years of life
before 100* for a person aged 65, and the *relative risk of dying before age 65* for a person aged 40, respectively. The reason for using different comparative measures for the two age groups is the fact that mortality is very low in the younger age group compared to the older, at least during recent periods.
...
print(xtable(dr,label = "tab:cumswtab", caption = "ANOVA, ages 40--64."),
...
##drold
print(xtable(drold,label = "tab:cumswtabold", caption = "ANOVA, ages 65--99."),
      type = "latex", comment = FALSE)
```
...
No 349-2008-6592) and Edvinsson was economically supported by the Swedish Research Council via the project *Old age, health and inequality:
Mortality from a life course perspective in Sweden 1986--2009* (grant No 421-2012-892). We thank the
...
The exposure in years for men and women respectively are shown in Table \@ref(tab:womenexp), and Table \@ref(tab:menexp).
...
print(xtable(ex[,,2], label = "tab:womenexp", caption = "Exposure in years by social class and period, women.", digits = 0), 
      type = "latex", comment = FALSE)
```

```{r exposuretab2, message = FALSE, cache = FALSE, results = 'asis'}
print(xtable(ex[,,1], label = "tab:menexp", caption = "Exposure in years by social class and period, men.", digits = 0), 
      type = "latex", comment = FALSE)
```
