...
```{r hiscskumw, fig.cap = "Exposure by social class (from the top: elite, middle, workers, farmers, none) and time period, women.", out.height = "14cm", fig.scap="Exposure by social class and time period, women.",fig.height=4}
...
```{r probswabs, fig.cap = "Absolute risk of dying before the age of 65, given survival to age 40, by period and social class.", fig.scap = "Absolute risk of dying before 65.", fig.height = 8, out.height = "14cm"}
source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
refclw <- 1 ## pdw[3, ]
plot(midper, pdw[1, ] / refclw, col = scol[1], type = "b", ylim = c(0.0, 0.55),
     xlab = "Period", xlim = c(1800, 2015), ##log = "y",
...
legend("bottomleft", legend = his, col = scol, lty = 1:5, pch = 1:5, cex = 0.7, bty = "n")
refclm <- 1 ##pdm[3, ]
plot(midper, pdm[1, ] / refclm, col = scol[1], type = "b", ylim = c(0.0, 0.55), xlab = "Period",
...
```{r probsw, fig.cap = "Relative risk of dying before the age of 65, given survival to age 40, by period and social class. Reference is worker.", fig.scap = "Relative risk of dying before 65.", fig.height = 8, out.height = "14cm"}
source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
refclw <- pdw[3, ]
plot(midper, pdw[1, ] / refclw, col = scol[1], type = "b", ylim = c(1/3, 3),
     xlab = "Period", xlim = c(1800, 2015), log = "y",
...
legend("topleft", legend = his, col = scol, lty = 1:5, pch = 1:5, cex = 0.7, bty = "n")
refclm <- pdm[3, ]
plot(midper, pdm[1, ] / refclm, col = scol[1], type = "b", ylim = c(1/3, 3), xlab = "Period",
...
# ##dr
knitr::kable(dr, caption = "Single term deletions, ages 30-64.", digits = c(0, 1, 2, 4))
...
We use the *expected remaining years of life at 65 up to age 100* (reported as mean age at death) as the summary measure, since the relative risk is a not so good measure when risks are high. First, looking at Table \@ref(tab:cumswtabold), it is obvious that the statistical significance regarding differences between groups over time is strong.
...
```{r cumsw2abs, fig.cap = "Mean age at death for a 65-year-old, by social class and period.", fig.scap = "Differences in expected years of life at 65", fig.height = 8, out.height = "14cm"}
##source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
mm <- 0 ##resw[3, ]
plot(midper, resw[1, ] - mm, col = scol[1], type = "b", ylim = c(75, 88), xlab = "Period",
...
legend("topleft", legend = his, col = scol, lty = 1:5, pch = 1:5, cex = 0.7, bty = "n")
mm <- 0 ##resm[3, ]
plot(midper, resm[1, ] - mm, col = scol[1], type = "b", ylim = c(73, 85), xlab = "Period",
...
```{r cumsw2, fig.cap = "Excess years of life (compared to worker) before 100 for a 65-year-old, by social class and period.", fig.scap = "Differences in expected years of life at 65", fig.height = 8, out.height = "14cm"}
##source("R/plotED.glm.R")
oldpar <- par(mfrow = c(2, 1))
midper <- c(1812.5, 1837.5, 1862.5, 1887.5, 1912.5, 1937.5, 1981, 1991, 2000.5, 2009.5)
mm <- resw[3, ]
