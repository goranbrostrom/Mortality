...
\title{Creating Data for "Social inequality in mortality among
  adults and elderly in northern Sweden 1801--2013"\footnote{Version 2017-08-27, with a longer time period.}}
...
\subsection{POPLINK data, 1801--1950}

In this version, we start at 1801-01-01. The POPLINK data cover 1801--1950. A covariate {\tt period} is added and used for splitting the data into calendar time periods of length 10 years each.
...
library(stringr)
load("../../Data/adult.rda")
adult <- calWin(adult, c(1801, 1951))
source("R/addPer.R")
addPer
adult <- addPer(adult, cuts = seq(1801, 1951, by = 25))
adult <- rc(adult)
## NEW hisclass, 6 + 7 --> 6:
...
print(xtable(round(y, 1), digits = 0, 
             caption = "Hisclass by time period, 1801--2013 (per cent)."))
@
...
1801--2013, we can remove all born 1710 and earlier and all born 1974 or
...
vb <- vb[vb$birthdate > 1711 & vb$birthdate < 1974, ]
vb <- ageWin(vb, c(40, 90))
starts <- seq(1711, 1981, by = 10)
...
save(vb0, file = "../data2/vb0.rda")
save(vb, file = "../data2/vb.rda")
@
...
save(vbTab, file = "../data2/vbTab.rda")
save(vb0Tab, file = "../data2/vb0Tab.rda")
@
...
loTab <- aggregate(lisaold[, c("event", "exposure")], 
                   lisaold[, c("sex", "age", "period", "urban", 
                               "civst", "income", "educ")],
                   FUN = sum)
save(loTab, file = "../data2/loTab.rda")
save(lisaold, file = "../data2/lisaold.rda")
@ 

\ldots and of course {\tt loTab.rda}.

Now let us put on \emph{partner} on {\bf lisaold}. First we need to fix the file {\bf partner} so that we can distinguish 
females and males. We also remove homosexual couples: We want to check a female's status compared to her male partner's.

<<partner>>=
load("../../Data/partner.rda")
partner$id1 <- pmin(partner$LINNEID1, partner$LINNEID2)
partner$id2 <- pmax(partner$LINNEID1, partner$LINNEID2)
partner <- unique(partner[, c("LINNEIDAR", "id1", "id2")])
indx <- match(partner$id1, lisaold$LINNEID)
partner$sex1 <- lisaold$sex[indx]
indx <- match(partner$id2, lisaold$LINNEID)
partner$sex2 <- lisaold$sex[indx]
partner <- partner[!(is.na(partner$sex1) | is.na(partner$sex2)), ]
partner <- partner[partner$sex1 != partner$sex2, ]
dim(partner)
partner$female <- ifelse(partner$sex1 == "female", partner$id1, partner$id2)
partner$male <- ifelse(partner$sex1 == "male", partner$id1, partner$id2)
partner <- data.frame(AR = partner$LINNEIDAR,
                      female = partner$female,
                      male = partner$male)
@

Next, put on partners on the females of {\bf lisaold}:

<<putonpartner>>=
key <- paste(lisaold$LINNEID, lisaold$AR)
partner$key <- paste(partner$female, partner$AR)
indx <- match(key, partner$key)
lisaold$partner <- partner$male[indx]
@

Next, put on partner's \emph{education} and \emph{income}:

<<putinceduc>>=
males <- lisaold[lisaold$LINNEID %in% lisaold$partner, ]
females <- lisaold[!is.na(lisaold$partner), ]
indx <- match(females$partner, males$LINNEID)
females$m.educ <- males$educ[indx]
females$m.income <- males$income[indx]
@

Tabulating female's income and partner's income (\emph{m.income}):

<<tabminc>>=
x <- with(females, table(income, m.income))
x
@

Tabulating female's education and partner's education (\emph{m.educ}):

<<tabmeduc>>=
x <- with(females, table(educ, m.educ))
x
@
