...
```{r frominddata}
load("~/Forskning/Data/inddata.rda")
load("~/Forskning/Data/sex.rda")
...
```{r styronper}
load("~/Forskning/Data/socstyr.rda")
...
They need to be of the right age (born on November 1, 1955 or earlier). We
get the birthdates from *inddata*.
...
They need to be of the right age (born on November 1, 1965 or earlier). We
get the birthdates from *inddata*.
...
This is no longer the last FoB we are using: The follow-up time this time
*is* ten years (as before), from November 1, 1990 to December 31, 2001, when
the *lisa* data is filling in.

**Update 2016-11-05:** Draw it to December 31, 2001, i.e. add 14 months! Reason: 
No hisclass in LISA before 1 Jan 2002!
...
They need to be of the right age (born on November 1, 1970 or earlier). We
get the birthdates from *inddata*.
...
lim <- as.numeric(toTime("1970-11-01"))
...
dd <- per$deathdate[indx]
fob90$event <- !(is.na(dd)) & (dd <= lim + 31 + 61 / 365)
##fob90$event <- !(is.na(dd)) & (dd < 1987) 
fob90$exit <- fob90$enter + 10 + 1 + 61 / 365 ## 10 + 1 + nov, dec: 1 year and 2 months.
