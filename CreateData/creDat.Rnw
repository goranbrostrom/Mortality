...
\title{Creating Data for "Social inequality in mortality among
  adults and elderly in northern Sweden 1850--2013"\footnote{Paper presented
    ...
    Chicago, Illinois
    November 17--20, 2016. Version 2016-09-19.}}
...
\begin{enumerate}
\item \'Elite, HISCLASS 1, 2, and 3, Higher and lower managers, and higher
  professionals. 
\item HISCALSS 4, 5, 6, and 8. Farmers, fishermen, and lower professionals, lower clerical, foremen.
\item Medium skilled workers, HnISCLASS 7.
...
if (file.exists("fobb.rda")){
    ...
    fobb <- addPer(fobb, cuts = seq(1961, 2001, by = 10))
    load("../../../Data/sex.rda")
    ...
    fobb <- fobb[, c("id", "birthdate", "sex", "enter", "exit", "event",
...
<<lisadata>>=
if (file.exists("liss.rda") & file.exists("lisaold.rda")){
    load("liss.rda")
    ##if (!is.null(liss$nyhisclass)){
    ...
    lisaold$urban <- lisaold$FORSAMLING %in% c(248001, 248002, 284004, 
                                         284005, 248011, 248201, 248204)
    liss$id <- liss$LINNEID
    liss <- liss[, c("id", "birthdate", "sex", "enter", "exit", "event",
                     "period", "urban", "civst", "hisclass", "ULORSAK")]
...
print(xtable(round(y, 1), digits = 0, caption = "Hisclass by time period, 1901--2013 (per cent)."))
@
...
vbTab <- aggregate(vb[, c("event", "exposure")], 
                   vb[, c("sex", "age", "period", "cohort", 
                          "urban", "civst", "hisclass40", "hisclass60")], 
                   FUN = sum)
...
Thus, the one data file to use is {\tt vbTab.rda}.
