...
three social classes, reorganized from Hisclass.
...
of the regions are included. The data file contains the variables on social class (HISCLASS) sex, urban/rural residence, birth date, death date, first and last date of observation and type of
...
2013. We stratify on HISCLASS and use the covariates urban/rural and {\tt   civst}. The cumulative
hazards at the end of the time period (65 if 40--64, 90 if 65--89) are the
...
Performing the same types of analyses as for all-cause mortality, we start by looking at cumulative hazards by {\sc Hisclass} at age 65, given survival to age 40. They are adjusted for {\tt marital status} and urban/rural residence. We present separate analyses for women and men.
...
In this section we focus on cardiovascular mortality among those that mostly have left the workforce, the age group 65--89 years. In accordance with the analyses for the age group 40--64, the model adjusts for {\tt civil status} and urban/rural residence and with separate analyses for women and men. We use as a summary measure the value of the cumulative hazards at
...
The analysis for the period 1990--2005 for the whole population 40 years and older is based on separate analyses with income and education as explanatory variables and with controls of period 
...
Education had a strong, statistically significant, effect on survival, as expected.
