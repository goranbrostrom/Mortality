...
Performing the same types of analyses as for all-cause mortality, we start by looking at cumulative hazards by {\sc Hisclass} at age 65, given survival to age 40. They are adjusted for {\tt marital status}. We present separate analyses for women and men.
