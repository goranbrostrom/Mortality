...
from Poplink are accessed for the period 1851--1950s.

The other large data-set is extracted from the Linnaeus database
\citep{malm10}. Linnaeus is based on different linked population registers from
...
year 1960--1990, with additional information on deaths from National Board of Health and Welfare. For the period 2002--2013 we use the information from the yearly population registers (LISA) together with death information from National Board of Health and Welfare. 
...
The classification scheme we use is  based on HISCLASS \citep{lema11}, a class-based system. The scheme uses HISCO for assigning categories. HISCO (Historical International Standard Classifications of Occupations) \citet{lema02}  is a classification system of historical occupations, categorizing occupations according to tasks and skills. We have merged the original 12 classes in HISCLASS, based on position on the labour market, ending up with the following three and an additional fourth category with unknown class:
...
Those that we first observe in the first included censuses often lack stated occupations. This is especially the case for women and elderly being retired. The proportion with missing values ({\tt NA}) included is presented in Table~\ref{tab:nana}. It was very low
...
and almost a third have missing information during the 1970s. As a consequence, we only report the results for men 40--64 for the 1960s and 1970s. For the most recent periods, the information is fairly complete.
...
Regarding the distribution of marital status over time, see Figure~\ref{fig:civstskum}.The figure indicate an under-registration of those married and and over-registration of previously married during the 1960s. (WHY?)
...
Education is here analysed only for the period 1990 to 2005. Years of education are given in 4 categories and is based on level of education according to categories in the LISA database at Statistics Sweden. It ranges from basic education (less than nine years schooling) at the lowest level, to high academic degree (PhD and similar) at the highest level. Which are the new categories?)
