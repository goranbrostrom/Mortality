...
2013, with the exception of the period 1951--1960 and separately for sex. We stratify on HISCLASS
and control for the covariates {\tt   civst} and urban/rural residence. The cumulative
...
Figure~\ref{fig:cumsw} shows the summed hazards of dying in the age interval 40--64 during the different periods. Obviously, and not surprisingly, mortality is decreasing over time for both sexes. Women have consistently better survival than men. This is a well-known phenomenon in Swedish
...
By \emph{standardising e} the levels in each time period for each class, we illustrate the relative risks in Figure~\ref{fig:cumswstand}. In the rest of the paper, we only present these 
...
2013 and . We stratify on HISCLASS
and use the covariates urban/rural and {\tt   civst}. The cumulative
hazards at the end of the time period (65 if 40--64, 90 if 65--89) are the
main target in our analysis: These numbers are used in the process of
graphically illustrating the development over time.
...
recent phenomenon. Furthermore, this transition and the class pattern of mortality differ depending on age group. Inequality in mortality appear clearer for the population at working age, while the differences are smaller among the elderly. 
