...
The classification scheme we use is  based on HISCLASS \citep{lema11}, a class-based system. The scheme uses HISCO for assigning categories. HISCO (Historical International Standard Classifications of Occupations) \citet{lema02}  is a classification system of historical occupations, categorising occupations according to tasks and skills. We have merged the original 12 classes in HISCLASS, based on position on the labour market, ending up with the following three and an additional fourth category with unknown class:
...
Figures~\ref{fig:hiscskumm} and \ref{fig:hiscumw} show the distribution (per cent) of exposure
...
Those that we first observe in the first included censuses often lack stated 
occupations. This is especially the case for women and elderly being retired. 
The proportion with missing values ({\tt NA}) is shown at the bottom of the 
figures. It was very low
during the periods until 1950, between 4 and 8 \% lacked information
allowing determination of class. This increased considerably during the first 
two decades of data from the Linnaeus database. During the 1960s, every second 
person lacks class category
and almost a third have missing information during the 1970s. As a consequence, 
we only report the results for men 40--64 for the 1960s and 1970s. For the most 
recent periods, the information is fairly complete.
...
retired. The model controls for period, marital
...
ages around 1960--1980 (see Table~\ref{tab:nana}), because that kind of 
information is simply missing to a large extent.



\section{The effect of Hisclass, 1851--1950, 1961--2013}
...
Figure~\ref{fig:cumsw} shows mortality levels during the different periods. 
Obviously, and not surprisingly, mortality is decreasing over time, and we can 
already identify some interesting social patterns in mortality which will be 
commented more on below. Apart from
...
However, we are mainly interested in illustrating the change in relative 
position among the HISCLASSES over time rather than the absolute levels, so we 
\emph{standardise} the levels in each time period. It is illustrated in
Figure~\ref{fig:cumswstand}. In the rest of the paper, we only present these 
...
The analysis for the period 1990--2005 is based on a separate analysis with 
income and education as explanatory variables and with controls of period 
and marital status. See Figure~\ref{fig:ploteduc}.
...
<<plotincheart, fig.cap="Cumulative hazards by income and sex, cardiovascular death, 1990--2013.">>=
oldpar <- par(mfrow = c(1, 2))
...
The WHO Commission on Social Determinants of Health framework for the social determinants of health discusses how socio-economic position impacts on health. The influences are not direct but works through intermediate determinants related to exposure and vulnerability \citep{who10}. People are placed within socio-economic and political contexts, as well as value systems in the society. The socio-economic position imply different access to resources, power and prestige, all aspects of crucial importance. The framework takes a holistic approach to the social determinants, including not only the effect of pure economic and material resources. The importance of understanding people in their context has also been emphasised by for example \citet{kri05}. What she calls embodiment refers to the recognition that human beings are simultaneously social beings and biological organisms.
