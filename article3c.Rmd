...
manifests itself through the facts that the marriage frequency varies social class and certain social classes are 
...
all <- aggregate(exm["exposure"], by = exm[c("civst", "urban", "age")], FUN = sum)
all
```
...
el <- aggregate(elite["exposure"], by = elite[, c("civst", "urban", "age")], FUN = sum)
el
```
