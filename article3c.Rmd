...
all <- aggregate(exm["exposure"], by = exm[c("civst", "urban")], FUN = sum)
all
```
...
el <- aggregate(elite["exposure"], by = elite[, c("civst", "urban")], FUN = sum)
el
```
...
Let us fit with and without weights. Without weights:
