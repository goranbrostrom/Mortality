...
calculated.  We have 2 sexes, 3 marital statuses, 12 periods, 2
urban/rural groups, 5 social classes,
14 age groups, and 22 cohorts; in all, 221,760 possible
...
combinations we end up with is 17,181. The data set is thus reduced in
size by 99 per cent.
...
A alternative, and preferable, approach is the marginal model, where the disturbace effect of confounders is 
...
orr <- aggregate(x = vb0Tab$exposure, by = vb0Tab[, c("period", "civst", "hisclass")], FUN = sum)
...
out <- 100 * out
xli <-  c(1812, 1837, 1862, 1887, 1912, 1932, 1978, 1982, 1988, 1995, 2005, 2011)
...
orr <- aggregate(x = vb0Tab$exposure, by = vb0Tab[, c("period", "urban", "hisclass")], FUN = sum)
...
legend("bottomleft", legend = levhis, lty = 1:5, col = scol, pch = 1:5)
```

### Inverse exposure weighting
...
As an illustration, take the period `1901-1925` with males. There are six combinations of civil status and urban/rural, and for 
each we calculate the total exposure time. The total distribution by confounders is

```{r exweight}
exm <- vb0Tab[vb0Tab$period == "1901-1925" & vb0Tab$sex == "male", c("age", "civst", "urban", "exposure", "event", "hisclass")]
all <- aggregate(exm[, c("exposure", "event")], by = exm[, c("civst", "urban")], FUN = sum)
all
```

and for the elite we have

```{r forelite}
elite <- exm[exm$hisclass == "elite", c("age", "civst", "urban", "exposure", "event", "hisclass")]
el <- aggregate(elite[, c("exposure", "event")], by = elite[, c("civst", "urban")], FUN = sum)
el
```

The weights are now 'all["exposure"]" / el["exposure"]`, that is

```{r wei}
wei <-  all$exposure / el$exposure
all$wei <- wei 
all$Expos <- all$exposure * wei
all$Eve <- all$event * wei
all
```

Put these weights on the data frame `elite`:

```{r putonel}
all$id <- paste(all$civst, all$urban, sep = "_")
elite$id <- paste(elite$civst, elite$urban, sep = "_")
indx <- match(elite$id, all$id)
elite$wght <- all$wei[indx]
```

Now repeat this for the other four hisclasses:

```{r therest}
middle <- exm[exm$hisclass == "middle", c("age", "civst", "urban", "exposure", "event", "hisclass")]
el <- aggregate(middle["exposure"], by = middle[, c("civst", "urban")], FUN = sum)
all$wei <- all$exposure / el$exposure
middle$id <- paste(middle$civst, middle$urban, sep = "_")
indx <- match(middle$id, all$id)
middle$wght <- all$wei[indx]
el
##
worker <- exm[exm$hisclass == "worker", c("age", "civst", "urban", "exposure", "event", "hisclass")]
el <- aggregate(worker["exposure"], by = worker[, c("civst", "urban")], FUN = sum)
all$wei <- all$exposure / el$exposure
worker$id <- paste(worker$civst, worker$urban, sep = "_")
indx <- match(worker$id, all$id)
worker$wght <- all$wei[indx]
el
##
farmer <- exm[exm$hisclass == "farmer", c("age", "civst", "urban", "exposure", "event", "hisclass")]
el <- aggregate(farmer["exposure"], by = farmer[, c("civst", "urban")], FUN = sum)
all$wei <- all$exposure / el$exposure
farmer$id <- paste(farmer$civst, farmer$urban, sep = "_")
indx <- match(farmer$id, all$id)
farmer$wght <- all$wei[indx]
el
##
non <- exm[exm$hisclass == "NA", c("age", "civst", "urban", "exposure", "event", "hisclass")]
el <- aggregate(non["exposure"], by = non[, c("civst", "urban")], FUN = sum)
all$wei <- all$exposure / el$exposure
non$id <- paste(non$civst, non$urban, sep = "_")
indx <- match(non$id, all$id)
non$wght <- all$wei[indx]
el
```
...
```{r puttog}
per1901 <- rbind(elite, middle, worker, farmer, non)
per1901$age <- factor(per1901$age)
#fit <- lm(exposure ~ civst * urban * age, data = per1901)
#pred <- predict(fit, type = "response")
#per1901$wght2 <- pred / per1901$exposure
```

Let us fit with and without weights. Without weights, see Figure \@ref(fig:testfit).

```{r testfit, fig.cap = "Survival by hisclass, men 1901-1925, non-weighted analysis.", fig.height = 4}
fit1 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "elite", ], family = poisson)
co1 <- fit1$coefficients
co1[2:length(co1)] <- co1[2:length(co1)] + co1[1]
co1 <- exp(co1)
##
fit2 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "middle", ], family = "poisson")
co2 <- fit2$coefficients
co2[2:length(co2)] <- co2[2:length(co2)] + co2[1]
co2 <- exp(co2)
##
fit3 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "worker", ], family = "poisson")
co3 <- fit3$coefficients
co3[2:length(co3)] <- co3[2:length(co3)] + co3[1]
co3 <- exp(co3)
##
fit4 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "farmer", ], family = "poisson")
co4 <- fit4$coefficients
co4[2:length(co4)] <- co4[2:length(co4)] + co4[1]
co4 <- exp(co4)
##
fit5 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "NA", ], family = "poisson")
co5 <- fit5$coefficients
co5[2:length(co5)] <- co5[2:length(co5)] + co5[1]
co5 <- exp(co5)
###
cuts <- seq(5, 55, by = 5)
x <- seq(0, 60, length = 1000)
s1 <- ppch(x, cuts = cuts, levels = co1[1:12], lower.tail = FALSE)
plot(x, s1, type = "l", col = scol[1], axes = FALSE, xlab = "age", ylab = "Surviving fraction after 30.")
axis(1, at = seq(0, 60, by = 10), labels = seq(30, 90, by = 10))
box()
axis(2, las = 1)
abline(h = 0)
abline(v = 0)
s2 <- ppch(x, cuts = cuts, levels = co2[1:12], lower.tail = FALSE)
lines(x, s2, col = scol[2], lty = 2)
s3 <- ppch(x, cuts = cuts, levels = co3[1:12], lower.tail = FALSE)
lines(x, s3, col = scol[3], lty = 3)
s4 <- ppch(x, cuts = cuts, levels = co4[1:12], lower.tail = FALSE)
lines(x, s4, col = scol[4], lty = 4)
s5 <- ppch(x, cuts = cuts, levels = co5[1:12], lower.tail = FALSE)
lines(x, s5, col = scol[5], lty = 5)
##
```

With weights, see Figure \@ref(fig:testfit2):


```{r testfit2, fig.cap = "Survival by hisclass, men 1901-1925, weighted analysis.", fig.height = 4}
fit1 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "elite", ], family = poisson, weights = wght)
co1 <- fit1$coefficients
co1[2:length(co1)] <- co1[2:length(co1)] + co1[1]
co1 <- exp(co1)
##
fit2 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "middle", ], family = "poisson", weights = wght)
co2 <- fit2$coefficients
co2[2:length(co2)] <- co2[2:length(co2)] + co2[1]
co2 <- exp(co2)
##
fit3 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "worker", ], family = "poisson", weights = wght)
co3 <- fit3$coefficients
co3[2:length(co3)] <- co3[2:length(co3)] + co3[1]
co3 <- exp(co3)
##
fit4 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "farmer", ], family = "poisson", weights = wght)
co4 <- fit4$coefficients
co4[2:length(co4)] <- co4[2:length(co4)] + co4[1]
co4 <- exp(co4)
##
fit5 <- glm(event ~ offset(log(exposure)) + age, data = per1901[per1901$hisclass == "NA", ], family = "poisson", weights = wght)
co5 <- fit5$coefficients
co5[2:length(co5)] <- co5[2:length(co5)] + co5[1]
co5 <- exp(co5)
###
