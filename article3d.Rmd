...
library(knitr)
opts_chunk$set(engine='R',fig.path='figs/art3d-',tidy=FALSE,fig.height=8.5,fig.width=8,strip.white=TRUE,cache=FALSE,echo=FALSE,message=FALSE,comment=NA,show.signif.stars = FALSE)##$
library(knitr)
library(eha)
library(xtable)
library(skum)
...
##
skume1 <- age.window(skume, c(40, 65))
...
As an illustration, take the period `2005-2013` with males, 65--99 years of age. There are six combinations of civil status and urban/rural, and for 
each we calculate the total exposure time. The total distribution by confounders is

```{r exweight, echo = TRUE}
...
plot(survfit(fit), mark.time = FALSE, col = scol, lty = 1:5, xlim = c(65, 100), xlab = "Age", ylab = "Survival", main = "Unweighted")
abline(h = 0)
abline(h = 0.5, lty = 3)
...
plot(survfit(fitw), mark.time = FALSE, col = scol, lty = 1:5, xlim = c(65, 100), xlab = "Age", ylab = "Survival", main = "Weighted")
abline(h = 0)
abline(h = 0.5, lty = 3)
...
The estimated medians are

```{r estmed}
knitr::kable(round(medi, 1))
```
