...
```{r readalldata}
source("R/get.file.R")
skel <- get.file("rawskel")
skel$region <- "ske"
skel$urban <- FALSE
vars <- c("id", "birthdate", "sex", "period", "urban", "civst", "hisclass", "enter", "exit", "event")
skel <- skel[, vars]
##
skume <- get.file("rawskume")
skume <- skume[, vars]
##
vb <- rbind(skel, skume)
vb <- vb[order(vb$id, vb$exit), ]
vb$id <- paste("vb", vb$id, sep = "_")
##
foli <- get.file("rawfoli")
foli <- foli[, vars]
```
