...
xxx <- rbind(xx, xy, xz)
yy <- round(prop.table(xxx, 1) * 100, 1)
barplot(t(yy)[5:1, ], col = c("red", "blue", "green", "black"), las = 2, ylab = "Per cent")
```
...
woutp <- plotED.glm(resw, col = scol, logScale = TRUE, relative = TRUE,
                    ylab = "H(65|40)", ylim = c(0.0, 0.6), main = "Women 30-64")##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = TRUE, relative = TRUE,
                    ylab = "H(65|40)", ylim = c(0.0, 0.6), main = "Men 30-64")##, exclude = 5:6)
par(oldpar)
```
...
resw <- pgetPHglm2(vb6599[vb6599$sex == "female", ],
            nper, labb)
resm <- pgetPHglm2(vb6599[vb6599$sex == "male", ],
            nper, labb)

oldpar <- par(mfrow = c(2, 1))
woutp <- plotED.glm(resw, col = scol, logScale = TRUE, relative = FALSE,
                    ylab = "H(90|65)", ylim = c(0.0, 10.0), main = "Women 65-99")##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = TRUE, relative = FALSE,
                    ylab = "H(90|65)", ylim = c(0.0, 10.0), main = "Men 65-99")##, exclude = 5:6)
par(oldpar)
```
...
woutp <- plotED.glm(resw, col = scol, logScale = TRUE, relative = TRUE, 
                    ylab = "H(90|65)",
                main = "Women 65-99", ylim = c(0, 0.6))##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = TRUE, relative = TRUE,
                    ylab = "H(90|65)",
                main = "Men 65-99", ylim = c(0, 0.6))##, exclude = 5:6)
par(oldpar)
```
...
dr <- drop1(fitt, test = "Chisq")
##knitr::kable(dr)
xtable(dr,label = "tab:cumswoldtab", caption = "ANOVA, all causes of death, 65-99.")
```
