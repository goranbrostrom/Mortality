...
##load("../Data/foli2.rda")
skel$urban <- FALSE
su <- rbind(skel, skume[, names(skel)])

vb0Tab <- rbind(su, foli[, names(su)])
...
vb0Tab <- aggregate(vb0Tab[, c("event", "exposure")],
                    ...
                    FUN = sum)
vb0Tab <- vb0Tab[!(vb0Tab$period %in% c("1961-1970", "1971-1975")), ]
vb0Tab$period <- factor(vb0Tab$period)
```
...
University. Due to data issues (a great deal of missing data in some variables, for example social class, and additional work required to define the exact geographical areas of analysis) we we restrict the analysis to the period 1976 onwards. The study period from 1976 to 2000 is constructed from
...
For the last period, 2002--2013, we use the yearly population registers from Statistics Sweden
...
being a scientific choice, is also a practical one. Social position during working age is defined at around the age of 40 but defined from the last occupation in working age for the elderly and retired population; i.e. from age 65 until death or last observation.
...
\item {\tt NA}, Not Available, unknown. 
\end{enumerate}
...
but still low enough to be used in our analyses (results not shown). If we disregard the very first period, however, there were very few with occupation missing until 1950. 

```{r hiscskumm, fig.cap = "Exposure by social class (from the top: elite, middle, workers, farmers, missing) and time period, men.", fig.scap="Exposure by social class and time period, men.", out.height = "14cm", fig.height=4}
...
##nper <- 4
nper <- length(labb)
resw <- getPHglm2(vb3064[vb3064$sex == "female", ],
            nper, labb)
resm <- getPHglm2(vb3064[vb3064$sex == "male", ],
            nper, labb)
```
...
illustrate the relative risks in Figure \@ref(fig:cumswstand). 
...
woutp <- plotED.glm(resw, col = scol, logScale = TRUE, relative = TRUE,
                    ylab = "H(65|30)", ylim = c(0.0, 0.6), main = "Women 30-64")##, exclude = 5:6)
moutp <- plotED.glm(resm, col = scol, logScale = TRUE, relative = TRUE,
                    ylab = "H(65|30)", ylim = c(0.0, 0.6), main = "Men 30-64")##, exclude = 5:6)
par(oldpar)
```
...
up until the first half of the 20th century, they have constantly decreased both in numbers as proportionally since then, and represent a very small minority at the enbd, particularly among women. 
...
accordance with previous analyses on other regions in northern Sweden and for other periods [@edli11; @edbr12]. The highest social class usually has comparatively low mortality (except for the first period when the statistical power was weak), while working-class women have high mortality. For women in the farming category, the mortality risks was at about the same levels as those of working-class women until the 1950s, while those in the middle category had somewhat lower mortality.   However, there is no substantial change during the studied period in the relative risk pattern between classes except for the results of the marginal farming category after 1976. Female mortality is characterised by a consistent social gradient. The group with unknown social class is distinguished by much higher mortality from the 1976 onwards. This might have to do with how we have defined social position for women before and after 1950. After 1975 it is based on their own occupation; lacking an occupation likely includes women who are more frail when it comes to health or are vulnerable due to social characteristics.

A completely different pattern appears among men, which is the most surprising finding in
our analyses. The group we expect to have the lowest mortality, if
we assume that access to vital resources and belonging to a high social class
determines survival, instead has by far the highest hazards of dying
during the 19th century and much of the 20th. Workers, on the
other hand, have comparatively low mortality until 1950, while mortality in the middle category represents an intermediate level. Farmers have similar levels as workers until the middle of the 20th century. The fact that we define social class as a time-dependent variable could otherwise lead to reverse causation -- those having bad health losing social position -- but instead, we find better survival among workers compared to the elite. 
...
differences are more compressed compared to the periods until 1950. A reversal of the class pattern in mortality has taken place, and the expected social gradient in mortality has developed. This latter pattern is also what we usually find in analyses of social health inequalities in present-day
...
```{r rawolddat,echo=FALSE}
rawskel <- get.file("rawskel")
rawskel$urban <- FALSE
rawskel <- rawskel[, c("id", "sex", "birthdate", "enter", "exit", "event", "period", "urban", "civst", "hisclass", "cohort")]
##
rawskume <- get.file("rawskume")
rawskume <- rawskume[, c("id", "sex", "birthdate", "enter", "exit", "event", "period", "urban", "civst", "hisclass", "cohort")]
raw <- rbind(rawskel, rawskume)
raw$id <- paste("pop", raw$id, sep = "_")
##
rawfoli <- get.file("rawfoli2")
rawfoli <- rawfoli[, c("id", "sex", "birthdate", "enter", "exit", "event", "period", "urban", "civst", "hisclass", "cohort")]
raw <- rbind(raw, rawfoli)
##raw <- raw[!(raw$period %in% c("1961-1970", "1971-1975")), ]
raw$period <- factor(raw$period)
raw <- age.window(raw, c(65, 100))
```


```{r getske6099}
vb6599 <- vb0Tab[as.numeric(as.character(vb0Tab$age)) >= 64.5, ]
vb6599$age <- factor(vb6599$age) # NOTE:: Change to '6599'!
##vb6599$hisclass <- vb6599$hisclass60
```

We now turn to the elderly, those who in most cases have left the workforce.
We use the *median remaining life at 65* (reported as median age at death) as the summary measure, here restricted to married people from the countryside. Looking at Table \@ref(tab:cumswtabold), it is obvious that the statistical significance is still strong.
...
knitr::kable(dr, caption = "Single term deletions, ages 65-99.")
##drold
...
The general pattern and development of mortality (Figures \@ref(fig:plotrawwom) and \@ref(fig:plotrawm)) do not differ much from the younger age group, of course representing a much
higher mortality level earlier in history. Mortality among women, as
expected, was lower than among men. We find a clear increase in survival, but
the fastest increase did not set in definitely until sometime after the
...
we find for the younger age group in line with the status-levelling hypothesis. For women the results are mixed. We do not find the same clear advantage for elite women during the period before 1950, but neither do we find any large disadvantage for working-class elderly women at that
time. In most periods the social differences were small, particularly between the workers, farmers and the middle category, and the elite women even had somewhat lower mean age at death during some periods in the 19th century. The social differences continued to be small until the last decades of the
20th century. It is only from around 1980 that it is possible to establish the expected social gradient in mortality.

For men we find indications of the same intriguing change among elderly as
for the adult male population aged 30--64 years, but at a much lower level and less consistent. Mean age at death was somewhat lower among elite men during the 19th century, a pattern that prevailed until the middle of the
...
workers of retirement age have slightly lower age at death. The difference between the worker, farmers and the middle group has been small in most periods.

```{r rawreswomen}
hc <- levels(raw$hisclass)
n.hc <- length(hc)
peri <- levels(raw$period)
n.peri <- length(peri)
med <- matrix(0, nrow = n.hc, ncol = n.peri)
for (i in 1:n.hc){
    fit <- coxph(Surv(enter, exit, event) ~ civst + urban + strata(period), data = raw[raw$sex == "female" & 
                                                                              raw$hisclass == hc[i], ])
    med[i, ] <- summary(survfit(fit, newdata = data.frame(civst = "married", urban = FALSE)))$table[, "median"]
}
```

```{r plotrawwom, fig.cap="Median age at death for a 65 year old married woman on the countryside by SES and period.", fig.height = 4, out.height = "10cm"}
x <- c(1812, 1837, 1862, 1887, 1912, 1937, 1963, 1969, 1975, 1981, 1987, 1993, 1999, 2006, 2012)
plot(x, med[1, ], col = scol[1], type = "b", lty = 1, pch = 1, xlab = "Period", ylab = "Age", ylim = c(70, 90), main = "Women", axes = FALSE)
axis(1, at = x, labels = peri, las = 2, cex.axis = 0.5)
axis(2, las = 1)
box()
abline(h = c(75, 80, 85), lty = 3)
for (i in 2:5){
    lines(x, med[i, ], col = scol[i], type = "b", lty = i, pch = i)
}
legend("bottomright", legend = hc, lty = 1:5, pch = 1:5, col = scol, cex = 0.6)
```
...
med <- matrix(0, nrow = n.hc, ncol = n.peri)
for (i in 1:n.hc){
    fit <- coxph(Surv(enter, exit, event) ~ civst + urban + strata(period), data = raw[raw$sex == "male" & 
                                                                              raw$hisclass == hc[i], ])
    med[i, ] <- summary(survfit(fit, newdata = data.frame(civst = "married", urban = FALSE)))$table[, "median"]
}
for (i in 1:n.hc){
    fit <- coxph(Surv(enter, exit, event) ~ strata(period), data = raw[raw$sex == "male" & 
                                                                              raw$hisclass == hc[i], ])
    med[i, ] <- summary(survfit(fit))$table[, "median"]
}
```

```{r plotrawm, fig.cap="Median age at death for a 65 year old married man on the countryside by SES and period.", fig.height = 4}
##x <- c(1812, 1837, 1862, 1887, 1912, 1937, 1975, 1985, 1995, 2004, 2011)
plot(x, med[1, ], col = scol[1], type = "b", lty = 1, pch = 1, xlab = "Period", ylab = "Age", ylim = c(70, 90), main = "Men", axes = FALSE)
axis(1, at = x, labels = peri, las = 2, cex.axis = 0.5)
axis(2, las = 1)
abline(h = c(75, 80, 85), lty = 3)
box()
