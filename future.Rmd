---
title: Is high social class always beneficial for survival? A study of northern Sweden 1801--2013 \footnote{Short; "Is high social class beneficial for survival?"}
author: Sören Edvinsson \footnote{email soren.edvinsson@umu.se}
  and Göran Broström \footnote{email goran.brostrom@umu.se (corresponding author)}
...
- \subsectionfont{\normalfont\itshape}
bibliography: mybib
abstract: "This is a test version with analyses using tabular data and pch models!! Focusing on two regions in northern Sweden 1801--2013, we challenge common
  notions of the assumed advantage in survival of belonging to a high social class. \\par\n\\textbf{Keywords:} Class reversal
...
library(knitr)
opts_chunk$set(engine='R',fig.path='figs_popstud/',tidy=FALSE,fig.height=8.5,fig.width=8,strip.white=TRUE,cache=FALSE,echo=FALSE,message=FALSE,comment=NA,show.signif.stars = FALSE,warning = FALSE)##$
library(knitr)
library(eha)
library(xtable)
library(skum)
...
lines(c(1976, 1976, 2014, 2014, 1976), c(40, 100, 100, 40, 40),
      col = "blue", lwd = 2)
#
popl <- vbTab[vbTab$period %in% levels(vbTab$period)[1:6],]
pn.exp <- sum(popl$exposure)
##round(sum(skume1$exit - skume1$enter) + sum(skume2$exit - skume2$enter))
pn.d <-  as.integer(sum(popl$event)) ##as.integer(sum(skume1$event) + sum(skume2$event))
#
...
#
foli <- vbTab[vbTab$period %in% levels(vbTab$period)[7:10], ]
##n.exp <- round(sum(foli1$exit - foli1$enter) + sum(foli2$exit ##-foli2$enter))
n.exp <- sum(foli$exposure)
...
xx <- with(vbTab[vbTab$sex == "male", ], tapply(exposure, list(period = period, hisclass = hisclass), sum))
...
xx2 <- with(vbTab[vbTab$sex == "female", ], tapply(exposure, list(period = period, hisclass = hisclass), sum))
...
```{r plottaalla,fig.height = 8, out.height = "18cm", fig.cap="Exposure by social class (from the top: elite, middle, workers, farmers, none), time period, and sex.", fig.scap="Exposure by social class, time period, and sex"}
cls <- rainbow(5, s = 1.0)
cls <- cls[c(2, 3, 1, 5, 4)]
cls[1] <- "black"
...
orr <- aggregate(x = vbTab$exposure, by = vbTab[, c("period", "civst", "hisclass")], FUN = sum)
...
orr <- aggregate(x = vbTab$exposure, by = vbTab[, c("period", "urban", "hisclass")], FUN = sum)
...
vb2_2005_men <- vb2Tab[vb2Tab$sex == "male" & vb2Tab$period == "2005-2013", ]
...
labb <- levels(vbTab$period)
nper <- length(labb)
his <- levels(vb1Tab$hisclass)
nhis <- length(his)
##
resm <- matrix(0, nrow = nhis, ncol = nper)
pdm <-  matrix(0, nrow = nhis, ncol = nper)
for (i in 1:nper){
    dat <- vb1Tab[vb1Tab$sex == "male" & vb1Tab$period == labb[i], ]
    ...
    ##x <- survfit(fit)
    for (j in seq_along(his)){
        fit <- tpchreg(event ~ offset(log(exposure)) + age, data = dat[dat$hisclass == his[j], ], cuts = seq(40, 65, by = 5))
        haz <- c(0, fit$hazards, fit$hazards[length(fit$hazards)])
        pdm[j, i] <- ppch(65, cuts = fit$cuts, levels = haz)
    }
...
for (i in 1:nper){
    dat <- vb1Tab[vb1Tab$sex == "female" & vb1Tab$period == labb[i], ]
    ##dat <- vb1[vb1$sex == "female" & vb1$period == labb[i], ]
    ##dat$wght <- getWeights(dat)
    ##fit <- coxph(Surv(enter, exit, event) ~ strata(hisclass), weights = wght, data = dat)
    ##x <- survfit(fit)
    for (j in seq_along(his)){
        fit <- tpchreg(event ~ offset(log(exposure)) + age, data = dat[dat$hisclass == his[j], ], cuts = cuts1)
        haz <- c(0, fit$hazards, fit$hazards[length(fit$hazards)])
        pdw[j, i] <- ppch(65, cuts = fit$cuts, levels = haz)
    }
...
fitt <- tpchreg(event ~ offset(log(exposure)) + age + urban + civst + sex * hisclass * period, cuts = cuts1,
             data = vb1Tab)
...
fittold <- tpchreg(event ~ offset(log(exposure)) + age + hisclass * period * sex + civst + urban, cuts = cuts2, data = vb2Tab)
...
```{r hazcalw2}
nper <- length(labb)
his <- levels(vb2Tab$hisclass)
nhis <- length(his)
##
resm <- matrix(0, nrow = nhis, ncol = nper)
for (i in 1:nper){
    dat <- vb2Tab[vb2Tab$sex == "male" & vb2Tab$period == labb[i], ]
    dat$wght <- getWeights(dat)
    for (j in seq_along(his)){
        fit <- tpchreg(event ~ offset(log(exposure)) + age, 
                       cuts = cuts2, 
                       data = dat[dat$hisclass == his[i], ])
        haz <- c(0, fit$hazards, fit$hazards[length(fit$hazards)])
        cat("lenght(haz) = ", length(haz), "\n")
        resm[j, i] <- mpch(levels = haz, cuts = fit$cuts)
    }
    ##fit <- coxph(Surv(enter, exit, event) ~ strata(hisclass), weights = wght, data = dat)
...
resw <- matrix(0, nrow = nhis, ncol = nper)
for (i in 1:nper){
    dat <- vb2Tab[vb2Tab$sex == "female" & vb2Tab$period == labb[i], ]
    dat$wght <- getWeights(dat)
        for (j in seq_along(his)){
        fit <- tpchreg(event ~ offset(log(exposure)) + age, 
                       cuts = cuts2, 
                       data = dat[dat$hisclass == his[i], ])
        haz <- c(0, fit$hazards, fit$hazards[length(fit$hazards)])
        resw[j, i] <- mpch(levels = haz, cuts = fit$cuts)
    }
