...
vb1Tab <- toTpch(vb1[, !(colnames(vb1) %in% c("id", "birthdate"))], cuts = seq(40, 65, by = 5))
##
vb2 <- rbind(skume2, foli2)
vb2$period <- factor(vb2$period)
vb2Tab <- toTpch(vb2[, !(colnames(vb2) %in% c("id", "birthdate"))], cuts = seq(65, 100, by = 5))
vb <- rbind(vb1, vb2)
...
fitt <- tpchreg(event ~ offset(log(exposure)) + age + urban + civst + sex * hisclass * period, cuts = seq(40, 65, by = 5),
             data = vb1Tab)
...
fittold <- tpchreg(event ~ offset(log(exposure)) + age + hisclass * period * sex + civst + urban, cuts = seq(65, 100, by = 5), data = vb2Tab)
...
for (i in 1:nper){
    ...
    for (j in seq_along(his)){
        fit <- tpchreg(event ~ offset(log(exposure)) + age, 
                       cuts = seq(65, 100, by = 5), 
                       data = dat[dat$hisclass == his[i], ])
        haz <- c(0, fit$hazards, fit$hazards[length(fit$hazards)])
        resm[j, i] <- mpch(levels = haz, cuts = 65:100)
    }
    ##fit <- coxph(Surv(enter, exit, event) ~ strata(hisclass), weights = wght, data = dat)
...
for (i in 1:nper){
    ...
    dat$wght <- getWeights(dat)
        ...
        fit <- tpchreg(event ~ offset(log(exposure)) + age, 
                       ...
                       data = dat[dat$hisclass == his[i], ])
        haz <- c(0, fit$hazards, fit$hazards[length(fit$hazards)])
        resw[j, i] <- mpch(levels = haz, cuts = 65:100)
    }
