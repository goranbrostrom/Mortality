...
vb1Tab <- toTpch(vb1[, !(colnames(vb1) %in% c("id", "birthdate"))], cuts = seq(40, 65, by = 1))
##
vb2 <- rbind(skume2, foli2)
vb2$period <- factor(vb2$period)
vb2Tab <- toTpch(vb2[, !(colnames(vb2) %in% c("id", "birthdate"))], cuts = seq(65, 100, by = 1))
vb <- rbind(vb1, vb2)
...
for (i in 1:nper){
    ...
    for (j in seq_along(his)){
        fit <- tpchreg(event ~ offset(log(exposure)) + age, data = dat[dat$hisclass == his[j], ], cuts = seq(40, 65, by = 1))
...
fitt <- tpchreg(event ~ offset(log(exposure)) + age + urban + civst + sex * hisclass * period, cuts = 40:65,
             data = vb1Tab)
...
fittold <- tpchreg(event ~ offset(log(exposure)) + age + hisclass * period * sex + civst + urban, cuts = 65:100, data = vb2Tab)
