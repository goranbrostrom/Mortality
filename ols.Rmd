...
So the *e60* for men from Skellefteå is `r round(x$coef[1], 2)` and for women it is `r round(x$coef[2], 2)` more years.
...
```{r standard}
fit <- coxph(Surv(enter, exit, event) ~ sex + region, data = sols)
survfit(fit)
```
